...
void InitMaze()
{
	...
	int col = rand() % MSZ;
	maze[row][col] = TARGET;
	Cell* pc1 = new Cell(row, col, nullptr);
	grays.push_back(pc1);
	...
	for(i=0;i<MSZ;i++)
		for (j = 0; j < MSZ; j++)
		{
			...
			case TARGET:
				...
				break;
			case BLACK:
				glColor3d(0.9, 0.7, 0.7); // light green
				break;
			case BLACK1:
				glColor3d(0.7, 1, 0.7); // light green
				break;
			case GRAY:
				glColor3d(0.6, 0.3, 0); // orange
				break;
			case PATH:
...
void RestorePath1(Cell* pc)
{
	while (pc->getParent() != nullptr)
	{
		maze[pc->getRow()][pc->getColumn()] = YELOOW;
		pc = pc->getParent();
	}
}
...
{
	if ((maze[row][column] == GRAY && maze[pcurrent->getRow()][pcurrent->getColumn()] == BLACK1) ||
		(maze[row][column] == GRAY1 && maze[pcurrent->getRow()][pcurrent->getColumn()] == BLACK))
	{
	...
	else // maze[row][column] is WHITE
	{
		...
		grays.push_back(pn);
		if (maze[pcurrent->getRow()][pcurrent->getColumn()] ==BLACK )

		{
			maze[row][column] = GRAY;
		}
		else
			maze[row][column] = GRAY1;
...
void RunBFSIteration()
{
	...
	else // grays is not empty
	{
		...
		if (maze[r][c] == START)
			...
			maze[r][c] = BLACK1; // paint it black1
		

		if (maze[r][c] == GRAY)
			maze[r][c] = BLACK; // paint it black
		else if (maze[r][c] == GRAY1)
			maze[r][c] = BLACK1; // paint it black1
		grays.erase(grays.begin());
		// add all white neighbors of pcurrent to grays
		// UP
		if (maze[r + 1][c] == SPACE || maze[r + 1][c] == GRAY1 || maze[r + 1][c] == GRAY)
			CheckNeighbor(r + 1, c, pcurrent);
		// DOWN
		if (runBFS && (maze[r - 1][c] == SPACE || maze[r - 1][c] == GRAY1 || maze[r - 1][c] == GRAY))
			CheckNeighbor(r - 1, c, pcurrent);
		// LEFT
		if (runBFS && (maze[r][c - 1] == SPACE || maze[r][c - 1] == GRAY1 || maze[r][c - 1] == GRAY))
